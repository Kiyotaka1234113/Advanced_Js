  ...
  MyApp.plugins.validation.validate = function (data, rules) {
    ...
    Object.keys(rules).forEach(function (field) {
      ...
      if (rule.required && (!value || value.trim() === '')) {
        errors.push({ field: field, message: rule.message || field + ' is required' });
        return;
      }
      ...
      if (value && rule.minLength && value.length < rule.minLength) {
        errors.push({ field: field, message: rule.message || field + ' is too short' });
      }
      if (value && rule.maxLength && value.length > rule.maxLength) {
        errors.push({ field: field, message: rule.message || field + ' is too long' });
      }
    });
    return {
    ...
    function match(path) {
      for (var i = 0; i < routes.length; i++) {
        ...
        if (matchResult) {
          ...
          return { handler: routes[i].handler, params: params };
        }
      }
      return null;
    }
